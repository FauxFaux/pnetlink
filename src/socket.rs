...
use libc::c_int;
use libc::{bind, getsockopt, recvfrom, send, setsockopt, socket};
...
impl NetlinkSocket {
    pub fn bind(proto: NetlinkProtocol, groups: u32) -> Result<NetlinkSocket> {
        ...
        let mut res = unsafe { socket(libc::PF_NETLINK, libc::SOCK_DGRAM, proto as i32) };
        if res < 0 {
    ...
    pub fn send(&mut self, buf: &[u8]) -> Result<usize> {
        use libc::c_void;
        let len = buf.len();
        let res = unsafe { send(self.fd, buf.as_ptr() as *const c_void, len, 0) };
        if res == -1 {
            return Err(Error::last_os_error());
        }
        Ok(res as usize)
    ...
    fn getsockopt_int(&mut self, level: c_int, option: c_int) -> Result<u32> {
        ...
        let res = unsafe {
            getsockopt(
                ...
                level,
                option as c_int,
                mem::transmute(&mut ffi_val),
                mem::transmute(&mut opt_len),
            )
        };
